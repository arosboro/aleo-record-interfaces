...
export function parse (res: string): AleoRecord {
  ...
  const resData: string = res
    .replace(/ {2}/g, '  "')
    .replace(/: /g, '": "')
    .replace(/,/g, '",')
    .replace(/\n}/g, '"\n}')

  const data: RestResponse = JSON.parse(resData)
  ...
  for (let i = 0; i < keys.length; i++) {
    ...
    const regex =
      /^(aleo[a-z0-9]{59})?([0-9]+)?(bool|i8|i16|i32|i64|i128|u8|u16|u32|u64|u128|field|group|scalar)?\.(private|public)$/m
